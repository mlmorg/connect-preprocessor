...
exports.prototype.compile = function (url, callback) {
  ...
  fs.readFile(filepath, { encoding: 'utf8' }, function (err, file) {
    if (err) {
      return callback('Error reading file `' + filepath + '`');
    }
    that.compiler.compile(file, _.clone(that.options.opts), function (err, c) {
      if (err) {
        return callback('Error compiling file `' + filepath + '`');
      }
      callback(null, c);
    });
  });
...
exports.prototype._compilerSetting = function (setting) {
  ...
  return this.compiler[setting](_.clone(this.options.opts));
};
